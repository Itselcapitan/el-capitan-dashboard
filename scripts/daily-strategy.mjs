...
- CRITICAL — IG AUDIENCE CONTEXT: Reid's existing IG follower base is ~54% Union College students (his school). This is a HYPER-LOCAL audience that watches but does NOT engage publicly (social pressure — they consider it "weird" to like/comment on a peer's content); however, they DO share his posts privately in DMs (both genuine fans and people making fun of him — both count as shares to the algo). Implication: standard IG engagement-rate benchmarks DO NOT apply to his graduated reels. A 63% skip rate on graduated reels is NORMAL behavior from a captive-but-passive audience that mostly watches once and moves on. Low like-to-view ratio is NOT a content problem — it is an audience-composition artifact. Stop recommending "improve hook quality" based on graduated-reel skip rate alone.
- CRITICAL — TRIAL REELS INTERPRETATION: Trial Reels are hidden from his existing followers and pushed to NON-followers only. This means Trial Reels are explicitly probing audiences OUTSIDE the Union College bubble (where IG is trying to find his DJ/producer niche). Consequences: (a) Trial Reel skip rate will ALWAYS look worse than graduated skip rate in early life — the algo is calibrating who to show them to and burning impressions on wrong-audience surfaces. (b) DO NOT diagnose Trial Reels as "broken hook" based on first-72h skip rate; the comparison `trialSkipRate vs graduatedSkipRate` is NOT apples-to-apples (graduated reels get warm-audience lift from the captive school cohort). (c) The real Trial Reels success metric is `follows-from-post` and `profile_visits` — if a trial reel drives even a handful of follows from non-Union-College accounts, it's a WIN regardless of skip rate, because it means the algo found a sliver of niche audience. (d) Track trial reels across their full lifecycle (>72h after graduation) before judging hook quality — a high-skip trial that later graduates and acquires niche followers IS the success state.
- CRITICAL — IG INSIGHTS RULE: When igInsightsSummary and per-post insight fields (skipRate, saves, shares, follows, profileVisits, avgWatchTimeMs) are present in the data, USE THEM as the primary diagnostic for content quality, BUT respect the audience-context and trial-reels rules above. Specifically: (1) For GRADUATED reels only: if skipRate > 75% AND shares are also low AND follows-from-post = 0, then flag as possible hook issue; below those compound thresholds, attribute to the Union College passive-audience effect. (2) For TRIAL reels: judge by follows-from-post and profile_visits, NOT skip rate. (3) follows-from-post is the true conversion metric — if a single post drove a disproportionate share of total follows, study its caption/format and replicate (this is especially meaningful when the source post is a Trial reel, because it means the algo found a niche audience). (4) saves are low across the catalog — reels are good for one-time consumption but lack replay value; recommend save-worthy formats (tutorials, save-this-mix carousels). (5) Never recommend "post more" generically when insights data is present — always tie recommendations to specific skip rates, share rates, or follows-from-post numbers. (6) When insights data is absent (scrape-ig-insights.mjs hasn't run yet), say so honestly rather than fabricating skip rates.
